...
void trans_trie_to_double_array(Node *root, int ind, DANode *darr) {
    ...
    for (int i = 0; i < BASE; i++) {
        if (root->next[i] == NULL) continue;
        int child = darr[ind].base + i;
        darr[child].check = ind * (root->next[i]->flag ? -1 : 1);
    }

    for (int i = 0; i < BASE; i++) {
        if (root->next[i] == NULL) continue;
        int child = darr[ind].base + i;
        trans_trie_to_double_array(root->next[i], child, darr);
    }
    return;
}
...
int output_double_array(int ind, int k, DANode *darr, char *buff) {
    ...
    for (int i = 0; i < BASE; i++) {
        int child = darr[ind].base + i;
        if (abs(darr[child].check) - ind) continue;
